...
Aquí tienes las respuestas claras, técnicas y fundamentadas para responderle a tu profesor con total seguridad. Estas explicaciones demuestran que entiendes la arquitectura más allá de solo escribir código.

1. ¿Por qué existen DTOs distintos para entrada y salida?
No usamos un solo DTO para todo porque los datos que necesito para CREAR algo son diferentes a los datos que necesito para MOSTRAR algo.

Principio de Responsabilidad:

Input (CreateUserDto): Contiene lo que el usuario escribe. Ejemplo: password. No contiene id ni createdAt porque esos datos los genera el sistema, no el usuario.

Output (UserResponseDto): Contiene lo que el sistema responde. Ejemplo: id, createdAt. NO debe contener el password por seguridad.

Validaciones Diferentes:

Al crear, el campo email es obligatorio.

Al actualizar (PATCH), el campo email es opcional.

Si usáramos un solo DTO, tendríamos conflictos de validación.

2. ¿Por qué la entidad nunca se devuelve al cliente?
Devolver la Entidad (@Entity) directamente es uno de los errores más graves en backend por tres razones:

Seguridad (Security Leak): Las entidades suelen tener datos sensibles que nunca deben salir del servidor, como contraseñas encriptadas, flags de "borrado lógico" (deletedAt) o tokens internos. Si devuelves la entidad, expones toda tu base de datos.

Desacoplamiento (Abstraction): Si devuelves la Entidad, tu frontend queda "amarrado" a tu base de datos. Si cambias el nombre de una columna en la BD, rompes el frontend. Usando DTOs, puedes cambiar la base de datos como quieras, y el DTO mantiene el "contrato" estable con el cliente.

Problema de Referencia Circular (Infinite Recursion): En relaciones complejas (ej. Usuario tiene Post, Post tiene Usuario), al convertir la entidad a JSON, el serializador entra en un bucle infinito y la aplicación falla. Los DTOs rompen este ciclo porque son estructuras planas.

3. ¿Cómo funciona el mapper?
El Mapper actúa como un traductor o puente entre las capas internas y externas para que no se mezclen.

Su función: Es tomar los datos de un objeto A y copiarlos selectivamente en un objeto B.

El flujo:

Entrada: Recibe un CreateDto (datos "crudos" del cliente) y lo convierte en una Entity para que la base de datos pueda guardarlo.

Salida: Recibe una Entity (datos "ricos" de la BD) y la convierte en un ResponseDto, filtrando los datos privados antes de enviarlos al cliente.

Beneficio: Gracias al Mapper, el Controlador nunca toca la Base de Datos directamente, y la Base de Datos nunca ve los formatos JSON del cliente. Mantiene las capas aisladas y limpias.