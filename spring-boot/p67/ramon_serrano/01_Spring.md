...
El endpoint /api/status responde a solicitudes GET y devuelve información en formato JSON. Esto ocurre porque el método con @GetMapping es ejecutado automáticamente cuando el servidor recibe una petición en esa ruta, y Spring Boot se encarga de convertir el resultado en JSON sin configuraciones adicionales.

Spring Boot, por su parte, facilita la creación de aplicaciones web porque incluye un servidor embebido (Tomcat). Al ejecutar el proyecto, este servidor se inicia automáticamente y permite que los endpoints funcionen sin necesidad de instalar o configurar un servidor externo. Gracias a esto, el desarrollo es más rápido y sencillo.
