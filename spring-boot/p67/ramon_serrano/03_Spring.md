...
Se utilizan DTOs distintos para entrada y salida porque la API solo debe recibir y devolver los datos necesarios, evitando exponer información interna o sensible del sistema.

La entidad representa el modelo interno de la aplicación y nunca se devuelve al cliente, ya que puede contener campos que no deben ser públicos y cambiar sin afectar la API.

El mapper se encarga de transformar la entidad en un DTO de respuesta (y viceversa), separando la lógica de conversión de los controladores, lo que mejora el orden, la claridad y el mantenimiento del código.
