...
utils/
```
![archivos](capturas/practicas02/EstructuraArchivos.png)
---
...
![alt text](capturas/practicas02/Archivo-Main.png)
---
...
```
![alt text](capturas/practicas02/Arbol-Terminal.png)
---
...
Los módulos son el mecanismo para aplicar la Arquitectura en Capas y el patrón MVCS de forma organizada y eficiente, lo cual es la base para una aplicación robusta.
* cómo se relacionan controllers, services y repositories
El flujo es una cascada de delegación que garantiza que cada componente solo hable con su capa adyacente:

    [Cliente] → Envía Petición HTTP.

    [Controlador] → Recibe, valida formato, y llama al Servicio.

    [Servicio] → Aplica lógica de negocio, y llama al Repositorio.

    [Repositorio] → Traduce a SQL, interactúa con la BD.

    [Base de Datos] → Ejecuta y devuelve Entidades.

    [Repositorio] → Devuelve Entidades al Servicio.

    [Servicio] → Finaliza la lógica de negocio, devuelve resultado al Controlador.

    [Controlador] → Genera la Respuesta HTTP (JSON + Status Code), y la envía al Cliente.


* qué problema evita mantener una estructura clara

Una estructura clara basada en la arquitectura de capas previene el caos, asegurando que la aplicación sea robusta y tenga una vida útil larga.
