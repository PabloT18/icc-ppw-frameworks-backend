...
- GET /api/users
  ![getUsers](assets/03/get-users.png)
- GET /api/users/:id
  ![getUser](assets/03/get-user.png)
- POST /api/users
  ![postUser](assets/03/post-user.png)
- PUT /api/users/:id
  ![putUser](assets/03/put-user.png)
- PATCH /api/users/:id
  ![patchUser](assets/03/patch-user.png)
- DELETE /api/users/:id
  ![deleteUser](assets/03/delete-user.png)

- GET /api/products
  ![getProducts](assets/03/get-products.png)
- GET /api/products/:id
  ![getProduct](assets/03/get-product.png)
- POST /api/products
  ![postProduct](assets/03/post-product.png)
- PUT /api/products/:id
  ![putProduct](assets/03/put-product.png)
- PATCH /api/products/:id
  ![patchProduct](assets/03/patch-product.png)
...
![usersController](assets/03/users-controller.png)

#### Products

![ProductsController](assets/03/products-controller.png)

### 3. Explicación breve

Incluyendo:

- **Por qué existen DTOs distintos para entrada y salida:** Los DTOs funcionan como filtros de seguridad. El DTO de entrada solo acepta los campos estrictamente permitidos. Así cada DTO tiene una responsabilidad específica: qué entra, qué sale, cómo se actualiza.

- **Por qué la entidad nunca se devuelve al cliente** La entidad contiene datos sensibles internos como `password` o `createdAt` que el cliente no debe ver. Si devolvemos la entidad completa, exponemos información crítica.

- **Cómo funciona el mapper:** El mapper transforma datos entre capas. El método `toEntity()` convierte un DTO de entrada a entidad interna (asignando campos que el cliente no envía, como password). El método `toResponse()` convierte la entidad a DTO de salida (copiando solo los campos permitidos).
