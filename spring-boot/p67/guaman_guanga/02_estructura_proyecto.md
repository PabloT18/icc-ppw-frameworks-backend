...
Tener los módulos separados en un proyecto Spring es importante porque permite organizar el código de forma lógica y ordenada, facilitando su comprensión y mantenimiento. Cada módulo cumple una responsabilidad específica, lo que ayuda a que el proyecto sea más escalable, ya que se pueden agregar nuevas funcionalidades sin afectar las existentes. Además, esta separación favorece el trabajo en equipo, pues varios desarrolladores pueden trabajar en distintas partes del sistema sin generar conflictos constantes.

* cómo se relacionan controllers, services y repositories
En Spring, estas capas siguen una arquitectura en capas:

> Controllers:
Son el punto de entrada de la aplicación. Reciben las solicitudes HTTP del cliente, validan los datos básicos y delegan la lógica al servicio correspondiente. No deben contener lógica de negocio.

> Services:
Contienen la lógica de negocio de la aplicación. Actúan como intermediarios entre los controllers y los repositories, procesando datos, aplicando reglas y coordinando operaciones.

> Repositories:
Se encargan del acceso a datos. Interactúan directamente con la base de datos mediante JPA, Hibernate u otro mecanismo de persistencia. Su función es guardar, consultar, actualizar y eliminar información.

* qué problema evita mantener una estructura clara
Mantener una estructura clara evita el código desordenado y difícil de mantener, conocido como código espagueti. Sin una estructura definida, la lógica de negocio puede mezclarse con el acceso a datos o con la capa de presentación, lo que provoca errores frecuentes, dificulta la depuración y hace que los cambios pequeños generen fallos en otras partes del sistema. Además, una estructura clara mejora la legibilidad, reduce el acoplamiento y facilita la realización de pruebas unitarias.
