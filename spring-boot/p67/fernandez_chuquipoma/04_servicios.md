...
Product
![Product Controller](./assets/UserController1.png)
...
Juan Fernandez
Usuario
![Usuario Service](./assets/UserServiceImpl1.png)

![Usuario Service](./assets/UserServiceImpl2.png)

Producto

![Producto Service](./assets/ProductServiceImpl.png)


![Producto Service](./assets/ProductServiceImpl2.png)


### 4. Explicación breve

* por qué se usa un servicio
Se usa para que el controlador no tenga que hacer todo el trabajo. El servicio es como el "cerebro" donde guardamos la lógica, como manejar la lista de usuarios o productos y controlar los IDs. Así, el código queda repartido de forma más inteligente.
* qué ventajas aporta MVCS
La ventaja principal es el orden. Al separar el Controlador (que atiende las rutas) del Servicio (que tiene la lógica), el proyecto es más fácil de mantener. Si en el futuro queremos cambiar la lista de mentira por una base de datos real, solo modificamos el servicio y el resto del programa sigue funcionando igual.
* por qué el controlador ahora está más limpio
Está más limpio porque ya no tiene que guardar listas ni saber cómo se crean o borran los datos. Ahora, gracias a la inyección de dependencias, el controlador solo tiene un "constructor" que recibe al servicio y lo llama cuando lo necesita. Su única función ahora es recibir la petición de Postman y pasarle la bola al servicio.
