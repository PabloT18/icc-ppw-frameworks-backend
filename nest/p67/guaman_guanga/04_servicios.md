...
![alt text](assets/imgF04.png)

### 2. Captura completa de

`products.service.ts`

![alt text](assets/imgF04-2.png)

![alt text](assets/imgF04-3.png)

### 3. Explicación breve:

* por qué se utiliza un servicio
Un **servicio** es la capa encargada de contener la **lógica de negocio** de la aplicación.

En este proyecto, el servicio:
- Maneja la creación, actualización, eliminación y modificación parcial de productos
- Aplica reglas de negocio
- Manipula los datos internos
- Centraliza la lógica reutilizable

Esto evita que los controladores tengan lógica compleja y permite:
- Reutilizar código
- Reducir duplicación
...
NestJS utiliza Inyección de Dependencias (Dependency Injection), lo que significa que el framework se encarga de crear y administrar las instancias de las clases.

El servicio se declara como @Injectable() y luego se inyecta automáticamente en el controlador a través del constructor:
...
Fácil reemplazo de implementaciones (por ejemplo, mocks en testing)

Código más limpio y mantenible

El controlador no crea el servicio, solo lo utiliza.

* por qué el controlador debe ser mínimo

El controlador debe encargarse únicamente de:

Recibir las peticiones HTTP

Obtener parámetros y cuerpos de la petición
...
No debe contener lógica de negocio ni manipulación directa de datos.

Ejemplo de controlador mínimo:
...
La arquitectura MVCS separa claramente las responsabilidades de cada capa:

| Capa       | Responsabilidad                  |
| ---------- | -------------------------------- |
| Model      | Entidades y estructuras de datos |
| View       | DTOs de salida y respuestas      |
| Controller | Manejo de peticiones HTTP        |
| Service    | Lógica de negocio                |

Beneficios del modelo MVCS:
...
Aplicación alineada con buenas prácticas backend

Cada capa cumple una única función, evitando dependencias innecesarias.
