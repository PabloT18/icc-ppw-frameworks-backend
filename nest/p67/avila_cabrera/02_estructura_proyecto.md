...
![users](assets/users.module.ts_D.jpeg)
...
NestJS se basa en una arquitectura modular, donde cada módulo representa un dominio funcional de la aplicación, como users, products o auth. Esta organización permite encapsular responsabilidades, configuraciones y dependencias dentro de cada módulo, logrando una estructura clara, ordenada y alineada con buenas prácticas empresariales.
#### ¿Qué relación hay entre controller, service y módulo?
El módulo es el elemento principal que agrupa y organiza los componentes relacionados con un dominio. Dentro de él se declaran los controllers, que se encargan de manejar las solicitudes HTTP y definir los endpoints, y los services, que contienen la lógica de negocio y son inyectados en los controllers mediante el sistema de inyección de dependencias de NestJS. Esta relación garantiza una correcta separación entre la capa de presentación y la lógica del sistema.
#### ¿Por qué separar dominios mejora la mantenibilidad?
Separar la aplicación por dominios mejora la mantenibilidad al reducir el acoplamiento entre componentes, facilitar la lectura del código y permitir que cada parte del sistema evolucione de forma independiente. Además, esta organización simplifica la detección de errores, el trabajo en equipo y la escalabilidad del proyecto, haciendo que el backend sea más robusto y fácil de extender.
