...
# **1. Introducción a las Relaciones en TypeORM (NestJS)**
...
| Uno a Muchos | `@OneToMany`/`@ManyToOne` | Usuario → Productos |
| Muchos a Muchos | `@ManyToMany` | Productos ↔ Categorías |
...
# RESUMEN ACTUALIZADO DE LA IMPLEMENTACIÓN DE RELACIONES ENTRE ENTIDADES

## 1. Entidades Base

- **UserEntity**: Solo define los campos básicos (`id`, `name`, `email`, `password`). No tiene relación bidireccional ni @OneToMany. Mantener simple.
- **CategoryEntity**: Inicialmente simple, preparada para evolucionar a N:N. Primero sin relación, luego agrega la relación ManyToMany.
- **ProductEntity**:
    - Fase 1: Relación ManyToOne con User y Category.
    - Fase 2: Relación ManyToMany con Category (usando tabla intermedia `product_categories`).

## 2. DTOs

- **CreateProductDto** y **UpdateProductDto**: Incluyen campos para IDs de usuario y categorías (en N:N, un Set de categoryIds).
- **ProductResponseDto**: Estructura anidada, incluye objetos anidados para usuario y lista de categorías.


## 3. Servicio ProductsService

- Valida existencia de usuario y categorías antes de crear/actualizar productos.
- Usa métodos de dominio para crear y actualizar productos.
- Mapea entidades a DTOs anidados para la respuesta.
...
1. **Captura de ProductEntity.ts** (con ambas versiones: 1:N y N:N)

    ![ProductEntity](assets/product.entity.png)

2. **Captura de ProductService.ts** (métodos create y update)

    ![ProductService1](assets/product.service1.png)
   
    ![ProductService2](assets/product.service2.png)
