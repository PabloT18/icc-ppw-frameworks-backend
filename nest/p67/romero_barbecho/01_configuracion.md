...
Entendí que en NestJS el @Controller sirve para manejar las rutas del servidor, es decir, es la parte que recibe las peticiones. El @Get simplemente indica qué método se ejecuta cuando alguien hace una petición de tipo GET. Los módulos ayudan a organizar el proyecto, como si fueran secciones donde se agrupan las partes relacionadas.

El servidor NestJS funciona escuchando en un puerto, y cada vez que llega una petición, busca qué controlador debe responder y devuelve lo que ese método tenga programado. Es como un sistema que recibe una solicitud, la procesa y responde.

También veo que NestJS se parece bastante a Spring Boot, porque los dos usan anotaciones para las rutas y tienen una estructura ordenada con controladores, servicios y módulos. Además, ambos manejan las peticiones de una forma muy parecida.
