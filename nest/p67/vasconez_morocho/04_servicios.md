...
![alt text](assets/product.controller.jpeg)

### 2. Captura completa de

`products.service.ts`

![alt text](assets/product.service.jpeg)

### 3. Explicación breve:

* Por qué se utiliza un servicio
Se usa para separar la lógica de negocio de la gestión de rutas lo cual nos permite que el código sea organizado, reutilizable y que cada clase tenga una sola responsabilidad.

* Cómo funciona la inyección de dependencias
NestJS crea automáticamente la instancia del servicio y la entrega al controlador a través del constructor. Lo cual evita que el controlador tenga que crear manualmente sus propias dependencias.

* Por qué el controlador debe ser mínimo
Su función es solo recibir peticiones y devolver respuestas. Al delegar la lógica al servicio, el controlador se vuelve más fácil de leer y mantener.

* Cómo esto beneficia al modelo MVCS
Beneficia al modelo MVCS Mejora el desacoplamiento entre capas. Lo cual permite modificar la lógica o la base de datos en el servicio sin necesidad de alterar el controlador, facilitando así el crecimiento del proyecto.
