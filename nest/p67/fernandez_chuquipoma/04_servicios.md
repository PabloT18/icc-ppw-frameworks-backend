...
### 1. Captura del `constructor` de `ProductsController`
  ...
  ![Usuario Servicio](./assets/nextUserService2.png)

  User Controller
  ![Usuario Controller modificado](./assets/usercontrollerModificado1.png)
...
Productos Service
  ...
  ![Productos Servicio](./assets/productService1.png)

  Productos Controller
  
  ![Products Servicio](./assets/productController1.png)

### 3. Explicación breve:

* por qué se utiliza un servicio
Básicamente para que el código no sea un relajo. Antes poníamos todo en el controlador, pero ahora el servicio es el que realiza el trabajo
* cómo funciona la inyección de dependencias
Es como pedir un favor automático. Usamos @Injectable() en el servicio para que NestJS sepa que está disponible. Luego, en el constructor del controlador, solo lo declaramos y NestJS nos "pasa" una copia del servicio lista para usar sin que nosotros tengamos que crearla manualmente.
* por qué el controlador debe ser mínimo
Porque su única misión es ser un "semáforo". Solo debe recibir la petición, mandarla al servicio y devolver la respuesta al cliente. Si el controlador es cortito, es mucho más fácil de leer y si algo falla, ya sabes que el problema está en la lógica del servicio.
* cómo esto beneficia al modelo MVCS
Nos ayuda a que el proyecto sea ordenado y escalable. Al tener todo separado por capas (rutas en el controlador y lógica en el servicio), si el día de mañana queremos usar una base de datos de verdad en lugar de una lista de mentira, solo cambiamos el servicio y el resto del código sigue funcionando igualito.
