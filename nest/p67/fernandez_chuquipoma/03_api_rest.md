...
Se usan para controlar exactamente qué datos se reciben del cliente y qué datos se le envían, permitiendo validar y filtrar la información en cada dirección.

* por qué la entidad no debe devolverse a la API

Porque la entidad suele contener datos internos o sensibles (como contraseñas o fechas de creación) que no deben ser expuestos al cliente final.

* cómo funciona el mapper

Funciona como un traductor que transforma un objeto de un tipo a otro, convirtiendo específicamente los DTOs en Entidades para lógica interna y las Entidades en DTOs de respuesta para la API.