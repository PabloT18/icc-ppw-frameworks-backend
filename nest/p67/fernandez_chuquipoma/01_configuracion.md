...
```
  ...
  ![Version de node.](./assets/nodeversion1.png)
  **Descripción de la Imagen:** Version de node usada para el proyecto 
...
```
  ...
  ![Comando ejecutado con la verificacion](./assets/nestAplication2.png)
  **Descripción de la Imagen:** Ejecucion del nest con la verificacion.
...
En navegador, Postman, Bruno o VS Code REST client.
  ...
  ![Despejar la web con el puerto 3000](./assets/nestWeb3.png)
  **Descripción de la Imagen:** Verificacion de la ejecucion de nuestra app
...
```
  ...
  ![Comando ejecutado para visualizar el ls](./assets/comandoLS4.png)
  **Descripción de la Imagen:** Ejecucion del comando ls para la verificacion.
...
Conceptos Clave
@Controller(): Define la clase como un controlador HTTP y establece la ruta base (ej. api/status).

@Get(): Marca un método para que responda a solicitudes GET en esa ruta.

Módulos (@Module): Son contenedores para organizar la aplicación en partes lógicas (controladores, servicios), mejorando asi su escalabilidad.

Funcionamiento del Servidor NestJS
El servidor inicia en main.ts, que usa el AppModule para cargar todos los componentes. NestJS luego:

Mapea todas las rutas definidas por los @Controller y @Get.

Gestiona automáticamente la Inyección de Dependencias, conectando servicios y controladores entre sí.

Comienza a escuchar peticiones en el puerto 3000.

Similitudes con Spring Boot
Ambos frameworks comparten una arquitectura similar y usan un diseño basado en clases y anotaciones/decoradores para definir los roles. Son ideales para construir aplicaciones escalables y estructuradas a nivel empresarial.

@Controller() en NestJS es similar a @RestController en Spring Boot.

@Get() en NestJS es similar a @GetMapping en Spring Boot.

Ambos se basan fuertemente en el patrón Controlador-Servicio y la Inyección de Dependencias.