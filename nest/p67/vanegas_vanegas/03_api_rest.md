...
Los DTOs de entrada se usan para controlar qué datos recibe la API desde el cliente, mientras que los DTOs de salida definen qué información se devuelve en las respuestas, evitando exponer datos innecesarios.
La entidad no debe devolverse directamente porque representa el modelo interno del sistema y puede contener información sensible o técnica que no debe ser visible para el cliente.
El mapper se encarga de transformar los datos entre DTOs y entidades, permitiendo separar la lógica interna del dominio de la información expuesta por la API y manteniendo el código organizado.