...
![Captura del constructor](./assets/constructor-mejora.png)
...
![services](./assets/service-producto.png)
### 3. Explicación breve:
...
## EXPLICACIÓN
En esta práctica se utiliza un **servicio** porque es el lugar adecuado para concentrar la **lógica de negocio** y las operaciones del CRUD, evitando que el controlador tenga demasiadas responsabilidades. La **inyección de dependencias** funciona haciendo que NestJS cree automáticamente una instancia del servicio (registrado en `providers`) y la entregue al controlador mediante el **constructor**, sin necesidad de instanciarlo manualmente. Por esta razón, el **controlador debe ser mínimo**: su tarea principal es recibir la petición HTTP, obtener parámetros o DTOs y delegar el trabajo al servicio. Esta forma de trabajo fortalece el modelo **MVCS**, ya que mantiene una separación clara entre capas (Controlador para rutas, Servicio para reglas y procesamiento, y DTO/Entidad/Mapper para manejo de datos), logrando un código más ordenado, escalable y fácil de mantener.
