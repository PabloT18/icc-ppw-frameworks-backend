...
En esta práctica aprendí que NestJS trabaja con una estructura modular, donde todo debe estar registrado en un módulo para funcionar correctamente. Los controladores se encargan de manejar las rutas y recibir las peticiones HTTP, mientras que decoradores como @Get() permiten definir endpoints específicos.

También comprendí que el servidor se inicia desde main.ts, donde la aplicación se levanta y queda a la espera de solicitudes en un puerto. Además, NestJS es muy parecido a Spring Boot, ya que ambos usan decoradores, inyección de dependencias y una estructura ordenada que facilita el desarrollo de aplicaciones limpias y mantenibles.
