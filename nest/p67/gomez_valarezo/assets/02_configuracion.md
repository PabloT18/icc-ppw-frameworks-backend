...
Entendí que @Controller define la ruta base y gestiona las peticiones entrantes, mientras que @Get asigna el verbo HTTP específico al método. Aprendí que la arquitectura de NestJS se basa en Módulos, lo que obliga a organizar y registrar cada componente (como el StatusModule) para que la aplicación funcione.

El servidor opera sobre Node.js y TypeScript, utilizando NestFactory para iniciar la aplicación y escuchar peticiones en el puerto 3000. Las similitudes con Spring Boot son evidentes: ambos utilizan decoradores (muy parecidos a las anotaciones de Java), siguen una arquitectura de inyección de dependencias y levantan un servidor embebido automáticamente, lo que hace que la estructura del proyecto se sienta muy familiar.
