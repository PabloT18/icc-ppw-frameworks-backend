...
![imagen](./assets/NestPrimero.jpeg)

### 2. Captura completa de

`products.service.ts`

![imagen](./assets/ServicioN.png)

### 3. Explicación breve:

¿Por qué se utiliza un servicio?

Se utiliza un servicio para concentrar la lógica de negocio en un solo lugar y evitar que el controlador tenga demasiada responsabilidad, haciendo el código más ordenado y mantenible.

¿Cómo funciona la inyección de dependencias?

La inyección de dependencias permite que NestJS cree y provea automáticamente las instancias necesarias (como los servicios) a los controladores, sin que el programador tenga que instanciarlas manualmente.

¿Por qué el controlador debe ser mínimo?

El controlador debe ser mínimo porque su función es recibir y responder solicitudes HTTP, no manejar lógica de negocio, lo que facilita el mantenimiento y las pruebas del sistema.

¿Cómo esto beneficia al modelo MVCS?

Esto beneficia al modelo MVCS porque separan claramente responsabilidades: el controlador gestiona las peticiones, el servicio la lógica, el modelo los datos y el mapper la transformación, logrando una arquitectura más limpia y escalable.
