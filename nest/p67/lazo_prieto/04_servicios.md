...
![contructor](assets/capturas/practicas04/Contructor.png)
...
![service-products](assets/capturas/practicas04/ProductsServices.png)

### 3. Explicación breve:

* por qué se utiliza un servicio
Se utiliza un servicio para separar la lógica de negocio del controlador. El servicio se encarga de manejar los datos y aplicar las reglas, mientras el controlador solo recibe las solicitudes y delega el trabajo.
* cómo funciona la inyección de dependencias
NestJS crea automáticamente una instancia del servicio y la inyecta en el controlador a través del constructor, sin que el controlador tenga que crearla manualmente.

* por qué el controlador debe ser mínimo
Porque su función es únicamente enrutar las peticiones HTTP y llamar al servicio, evitando lógica de negocio y manteniendo el código más claro y ordenado.

* cómo esto beneficia al modelo MVCS
Permite una separación clara de responsabilidades entre controlador, servicio y modelo, lo que facilita el mantenimiento, las pruebas y la escalabilidad de la aplicación.
